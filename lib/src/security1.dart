...
class Security1 implements ProvSecurity {
  ...
  Future<SessionData> setup0Response(SessionData responseData) async {
    ...
    _verbose('setup0Response:Device public key ${devicePublicKey!.bytes.toString()}');
    _verbose('setup0Response:Device random ${deviceRandom.toString()}');

    final sharedKey = await x25519.sharedSecretKey(keyPair: clientKey!, remotePublicKey: devicePublicKey!);
    var sharedK = Uint8List.fromList(await sharedKey.extractBytes());
    ...
    if (pop != null) {
      ...
      _verbose('setup0Response: pop: $pop, hash: ${hash.bytes.toString()} sharedK: ${sharedK.toString()}');
    }
    await crypt.init(sharedK, deviceRandom!);
    _verbose('setup0Response: cipherSecretKey: ${sharedK.toString()} cipherNonce: ${deviceRandom.toString()}');
    return setupResp;
  }
